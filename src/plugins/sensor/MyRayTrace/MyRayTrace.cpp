...
#include <new-scrimmage-plugin/plugins/sensor/MyRayTrace/MyRayTrace.h>
...
REGISTER_PLUGIN(scrimmage::Sensor,
                scrimmage::sensor::MyRayTrace,
                MyRayTrace_plugin)

namespace scrimmage {
namespace sensor {

std::vector<MyRayTrace::PCRay> MyRayTrace::PointCloud::get_rays() {
    std::vector<MyRayTrace::PCRay> return_rays;
    for (auto &ray : rays) {
        // Make a copy so can be used cleanly
        return_rays.push_back(MyRayTrace::PCRay(ray.azimuth_rad, ray.elevation_rad));
    }
    return return_rays;
}


MyRayTrace::MyRayTrace() : max_range_(0), min_range_(0), max_sample_rate_(0){}

void MyRayTrace::init(std::map<std::string, std::string> &params) {
    ...
    pub_ = advertise("LocalNetwork", "MyRayTrace");
}

bool MyRayTrace::step() {
    // pub_->publish(std::make_shared<MessageBase>());
    return true;
}

std::vector<MyRayTrace::PCRay> MyRayTrace::rays() {
    std::vector<MyRayTrace::PCRay> return_rays;
    for (auto &ray : rays_) {
        // Make a copy so can be used cleanly
        return_rays.push_back(MyRayTrace::PCRay(ray.azimuth_rad, ray.elevation_rad));
