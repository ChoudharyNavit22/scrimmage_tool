...
#include <new-scrimmage-plugin/plugins/interaction/MyMap2D/MyMap2D.h>
#include <new-scrimmage-plugin/plugins/interaction/MyMap2D/MyMap2DInfo.h>
...
REGISTER_PLUGIN(scrimmage::EntityInteraction,
                scrimmage::interaction::MyMap2D,
                MyMap2D_plugin)

namespace scrimmage {
namespace interaction {

bool MyMap2D::init(std::map<std::string, std::string> &mission_params,
                               std::map<std::string, std::string> &plugin_params) {
    pub_shape_gen_ = advertise("GlobalNetwork", "ShapeGenerated");
    pub_map_2d_info_ = advertise("GlobalNetwork", "MyMap2DInfo");
...
bool MyMap2D::step_entity_interaction(std::list<sc::EntityPtr> &ents,
                                                  double t, double dt) {
    if (!map_info_published_) {
        map_info_published_ = true;

        // Populate and publish the map information
        auto msg_map2d = std::make_shared<sc::Message<MyMap2DInfo>>();
...
std::list<cv::Rect> MyMap2D::find_rectangles(cv::Mat &img, int threshold) {
